                ...
                foreach($rules as $rule => $ruleValue) {
                    ...
                    if($rule === 'required' && empty($value)) {
                        ...
                        switch($rule) {
                            ...
                            case 'select':
                                if($value === $ruleValue) {
                                    $this->addErrors(["Please select {$display}", $input]);
                                } 
                            break;
                            ...
                            case 'isUniqueStoreName':
                                if($this->db->findFirst('stores', ['conditions' => ['store_name = ?'], 'bind'=>[$value]])) {
                                    $this->addErrors(["{$value} already exist in the database", $input]);
                                } 
                            break;

                            case 'isUniqueStoreNameById':
                                if($this->db->findFirst('stores', ['conditions' => ['store_id != ? AND store_name = ?'], 
                                                                  'bind'=>[Session::get(STORE_SESSION_NAME), $value]])) {
                                    $this->addErrors(["{$value} already exist in the database", $input]);
                                } 
                            break;

                            case 'isUniqueBuyer':
                                if($this->db->findFirst('buyers', ['conditions' => ['email = ?'], 'bind'=>[$value]])) {
                                    $this->addErrors(["{$value} already exist in the database", $input]);
                                } 
                            break;
                            ...
                            case 'size':
                                if(($_FILES[$input]['size'] > ($ruleValue * 1000000))) {
